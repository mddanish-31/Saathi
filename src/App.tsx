...
export const App: React.FC = () => {
  ...
  const parseRoute = (pathWithSearch: string) => {
    ...
    // 6c. Subcategory A6: Wedding Venues
    if (pathname === '/categories/weddings-events/wedding-venues') {
      return { type: 'wedding-venues' };
    }
    const a6CategoryMatch = pathname.match(/^\/categories\/weddings-events\/wedding-venues\/([^/]+)$/);
    if (a6CategoryMatch) {
      const a6CategorySlugs = ['marriage-halls', 'banquet-halls', 'venues', 'resorts', 'lawns', 'community-halls'];
      if (a6CategorySlugs.includes(a6CategoryMatch[1])) {
        return { type: 'wedding-venues-category', categorySlug: a6CategoryMatch[1] };
      }
    }
  ...
  const renderPage = () => {
    ...
    switch (route.type) {
      ...
      case 'wedding-venues':
        return <WeddingVenuesPage onNavigate={navigate} />;
      case 'wedding-venues-category':
        return <WeddingVenuesPage activeCategorySlug={route.categorySlug} onNavigate={navigate} />;
